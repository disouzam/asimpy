...
class LateMergeCar(Process):
    ...
    async def run(self):
        ...
        target = self.lane1 if len(self.lane1._items) <= len(self.lane2._items) \
                 else self.lane2
        if target.is_full():
            self.blocked.append(1)
            return
...
class ArrivalStream(Process):
    def init(self, lanes: list[Queue], sojourn_times: list, blocked: list,
             zipper: bool):
        self.lanes = lanes
        self.sojourn_times = sojourn_times
        self.blocked = blocked
    ...
    async def run(self):
        while True:
            ...
            if self.zipper:
                LateMergeCar(self._env, self.lanes[0], self.lanes[1],
                             self.sojourn_times, self.blocked)
            else:
                EarlyMergeCar(self._env, self.lanes[0],
                               self.sojourn_times, self.blocked)
...
print("  " + "-" * 52)
print(f"  {'Total buffer (cars)':<22}  {early['total_buffer']:>12}  "
      f"{late['total_buffer']:>12}")
print(f"  {'Throughput (cars/unit)':<22}  {early['throughput']:>12.4f}  "
      f"{late['throughput']:>12.4f}")
print(f"  {'Blocked cars %':<22}  {early['blocked_pct']:>11.1f}%  "
      f"{late['blocked_pct']:>11.1f}%")
print(f"  {'Mean sojourn time':<22}  {early['mean_sojourn']:>12.3f}  "
      f"{late['mean_sojourn']:>12.3f}")
print()
print(f"  Late merge has {late['throughput']/early['throughput']:.3f}x the throughput "
